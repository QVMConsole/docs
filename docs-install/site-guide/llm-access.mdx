...
sidebar_position: 2
title: 大模型阅读通道
description: 为文档提供一个供大模型阅读的地址：LLM 可请求 URL 获取标准 Markdown 文章，按问题精确检索
---
...
> 请先访问 https://qvmcdocs.xiaozhuhouses.asia/llms.txt 获取文档目录，然后根据我的问题「如何配置端口转发？」选择最相关的文章阅读，再基于文档内容回答。
...
:::note 需要把站点部署到公网才能给大模型用吗？
支持联网检索的大模型（如 DeepSeek、ChatGPT、Claude 等的联网模式）需要能访问站点地址。公网部署后可直接使用；本地 `localhost` 地址仅用于开发预览验证。
:::

:::note 改文档后地址会变吗？
会实时同步。开发模式保存即更新；每次构建发布时重新生成。文章地址以文档 id 为准，仅重命名文件才会导致地址变化。
:::

:::note 为什么文章返回的是 .md 而不是 .mdx？
为了让大模型读到纯净、标准的 Markdown。MDX 专有语法（组件标签、import 语句等）已自动转换或剥离，正文内容保持不变。
:::

:::note 开发预览时删除文章后地址还能访问？
磁盘上的生成文件会立即清理；受 Docusaurus 开发服务器内存缓存机制影响，本地预览可能继续返回旧内容直到重启，这是开发服务器本身的固有行为。正式构建（`pnpm build` / `build.ps1`）产物不会包含已删除的文章。
:::

:::note 大模型会读到未完成的内容吗？
不会。标记为 `draft` 的文档自动排除；正式发布前请勿移除 `draft` 标记。
:::