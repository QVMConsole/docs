...
sequenceDiagram
    ...
    Main->>Cfg: Init() 加载默认配置
    Main->>Log: InitWithConsoleConfig() 初始化日志
    Main->>DB: InitDB() 初始化数据库（GORM + SQLite）
    Main->>Cfg: LoadFromDB(savedSettings) 合并持久化配置
    Main->>RPC: InitLibvirtRPC() 建立 go-libvirt 连接
    ...
    Main->>Sched: StartStatsCollector / 气球调度器 / 调度事件清理 / 端口探测 / 定时任务 / JWT 密钥轮转 / 上传会话清理
    Main->>Sched: SyncSSHDenyConfig / EnsureAllActiveUsersDefaultSecurityGroup
    Main->>Sched: EnsureSystemBaseNetwork / 桥接恢复 / 端口转发恢复 / VPC 恢复 / 公网 IP 规则恢复
...
flowchart TD
    ...
    ReqFilter --> ReqGuard[请求守卫中间件<br/>补充防护]
    ReqGuard --> RateLimit[限流中间件<br/>API 请求频率控制]
    RateLimit --> Auth[认证中间件<br/>JWT/API Key 验证]
    Auth --> PwdChange[强制改密中间件]
    PwdChange --> Admin[管理员中间件<br/>权限检查]
    Admin --> Elastic[ElasticCloud 中间件<br/>轻量云用户限制]
