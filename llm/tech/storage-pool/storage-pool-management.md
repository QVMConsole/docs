...
> **访问权限** 存储池列表、详情、配置、默认设置、格式化、分区与 LVM 卷操作均需管理员权限；`all-isos`（全局 ISO 列表）与 `vm-targets`（创建 VM 可选存储目标）对普通用户开放。格式化、分区、清盘、建卷、删卷为高风险操作，需要二次验证并以异步任务执行。
...
| 字段                 | 说明                           | 示例                                                            |
| ------------------ | ---------------------------- | ------------------------------------------------------------- |
| **device\_path**   | 设备路径                         | `/dev/sda`、`/dev/nvme0n1`                                     |
...
> **危险操作** 格式化挂载会**清空磁盘上所有数据**，此操作不可逆。请务必确认数据已备份后再执行。
...
> **新分区不会自动格式化** 创建分区后需对该分区再执行「格式化挂载」才能用作存储池。操作以异步任务执行（`storage_create_partition`）。
...
> 原文路径：/docs/tech/storage-pool/storage-pool-management（本文由 QVMConsole 文档站自动生成，供大模型阅读）
