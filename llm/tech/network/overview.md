...
> **服务异常处理** 如果任何服务状态显示异常（状态接口会附带 issues 与修复建议），请使用"一键检测"进行诊断，或使用"一键修复"（高风险操作，异步任务 `ovs_repair`）尝试自动恢复。
...
概览页集成端口镜像状态卡：展示启用状态、健康状态、源接口入/出方向复制计数与目标交换机收包统计，提供启用/更新与停用入口（均为高风险异步任务）。功能原理与配置方法详见 [端口镜像](/docs/tech/network/port-mirror)。
...
| **桥接直通网桥**    | 绑定物理网卡的 OVS 网桥，供物理直通交换机使用                     |

直通网桥有两种创建途径：

1. **自动创建（推荐）**：创建[物理直通交换机](/docs/tech/network/vpc-switch)并选择上联网卡时，首个交换机会自动创建并拥有直通网桥，后续同网卡交换机共享该网桥
2. **手动创建**：在本页「创建桥接网桥」对话框中预先创建（网桥名称 + 物理网卡 + 是否迁移宿主机 IP），供后续绑定使用
...
> **网络中断风险** 创建/删除桥接直通网桥会短暂中断该物理网卡上的网络连接（尤其是承载宿主机默认路由的网卡），请在维护窗口操作。
...
| **IP 地址**               | CIDR 地址列表（IPv4/IPv6）                          |
| **默认路由**                | 是否为默认路由网卡（承载默认路由的网卡会给出风险提示）                   |
| **OVS 网桥 / 端口**         | 已加入 OVS 时显示所属网桥与端口名                           |
| **托管网桥**                | 已被面板管理的网桥标识                                   |
| **直通占用**                | 已被物理直通交换机占用时显示所属交换机名与已用 VLAN ID 列表，以及是否仍可共享直通 |

## 接口 IP / DNS 配置

管理员可以在物理网卡或网桥上直接配置 IPv4 / IPv6 地址、网关与 DNS（`GET/PUT /network/interfaces/:name/config`）：

| 配置项         | 说明                                       |
| ----------- | ---------------------------------------- |
| **IPv4 地址** | 一个或多个 CIDR 地址（换行或逗号分隔）                   |
| **IPv4 网关** | 默认网关 IP（合法 IPv4）                         |
| **IPv6 地址** | 一个或多个 IPv6 CIDR 地址（换行或逗号分隔）              |
| **IPv6 网关** | IPv6 默认网关（如 `fe80::1` 或全局地址）             |
| **metric**  | IPv4 / IPv6 各自的路由 metric                 |
| **DNS**     | 一个或多个 DNS 服务器（空格或逗号分隔，支持 IPv4 与 IPv6 地址） |
| **清除配置**    | 移除地址/DNS 与面板托管的静态配置（IPv4 与 IPv6 一并清除）    |

保存时要求 IPv4 与 IPv6 地址**至少配置一项**；纯 IPv6 单栈配置同样受支持（向后兼容既有 IPv4 功能）。
...
持久化方式：托管网桥更新数据库记录并重写开机恢复脚本（网桥记录新增 `host_addrs6` / `host_gateway6` / `host_metric6` 字段保存 IPv6 配置）；独立物理网卡写入 systemd-networkd 静态配置（`/etc/systemd/network/10-kvm-console-static-<网卡>.network`）并 `networkctl reload`。
...
| **一键修复**（`POST /ovs/repair`） | 高风险操作：确保 OVS 与 dnsmasq 就绪 → 重建网桥与网关 → 恢复 NAT/FORWARD/ip\_forward → 重新检查 |

> **使用建议** 建议在执行一键修复前先了解当前网络配置；修复以异步任务执行，进度可在任务中心查看。
...
> 原文路径：/docs/tech/network/overview（本文由 QVMConsole 文档站自动生成，供大模型阅读）
