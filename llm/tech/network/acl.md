...
> **预览用途** ACL 预览功能主要用于审计和调试。在应用规则前，建议先预览规则内容，确保规则符合预期。
...
| 操作         | 说明                               | 影响   |
| ---------- | -------------------------------- | ---- |
| **预览**     | 按当前安全组规则生成 nftables 规则文本（支持一键复制） | 只读   |
| **刷新预览**   | 重新生成预览内容                         | 只读   |
| **应用 ACL** | 危险操作二次确认后，将预览规则加载到系统（需高风险二次验证）   | 立即生效 |

> **应用影响** 应用 ACL 规则会立即影响所有绑定安全组的虚拟机的网络访问。建议在业务低峰期进行操作，并提前通知相关用户。
...
| 组件     | 说明                     | 用途              |
| ------ | ---------------------- | --------------- |
| **表**  | kvm\_console\_vpc\_acl | 存储所有 VPC ACL 规则 |
| **链**  | ingress                | 处理入站流量          |
| **链**  | egress                 | 处理出站流量          |
| **规则** | 具体的匹配规则                | 控制网络访问          |
...
> **同步建议** 每次修改安全组规则后，建议重新预览和应用 ACL 规则，确保安全策略立即生效。

## ACL 规则管理

ACL 管理模块提供以下操作（均为管理员权限）：

| 操作       | 说明                              |
| -------- | ------------------------------- |
| **预览**   | 预览生成的 nftables 规则（代码块展示，支持一键复制） |
| **刷新预览** | 重新生成预览内容                        |
| **应用**   | 二次确认 + 高风险验证后将规则加载到系统           |

> **规则备份** 预览内容支持一键复制为 nftables 文本，可自行保存用于审计与备份；面板不提供独立的规则导出接口。已生效的规则也可在宿主机通过 `nft list table inet kvm_console_vpc_acl` 查看。
...
3. **备份规则**：复制预览内容或宿主机 nft 输出自行留档，便于恢复
...
> **安全警告** ACL 规则直接控制网络访问权限，错误的规则配置可能导致安全漏洞或服务中断。建议在测试环境验证后再应用到生产环境。
...
| **nftables 状态** | 宿主机执行 `nft list table inet kvm_console_vpc_acl` | 验证已生效规则 |
| **日志查看**        | 查看系统日志                                          | 追溯问题原因  |

> **诊断建议** 遇到 ACL 问题时，建议先使用 ACL 预览功能检查规则配置，然后在宿主机检查 nftables 表是否存在及其内容。如果问题仍然存在，可以查看系统日志获取更多信息。

---

> 原文路径：/docs/tech/network/acl（本文由 QVMConsole 文档站自动生成，供大模型阅读）
