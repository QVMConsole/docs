...
> **迁移中状态** 虚拟机跨节点迁移期间会显示为「迁移中」，此时电源操作、配置修改和删除会被阻止。
...
| ------- | ------------- | -------------------------- |
| **创建中** | 正在通过任务队列创建虚拟机 | 无                          |
| **关机**  | 虚拟机已停止        | 开机、编辑、删除、克隆、重装、救援          |
| **运行**  | 虚拟机正在运行       | 关机、强制断电、重启、重置、VNC/SPICE 连接 |
| **迁移中** | 虚拟机正在跨节点迁移    | 等待迁移完成                     |
...
> 原文路径：/docs/tech/feature-analysis/virtualization-overview（本文由 QVMConsole 文档站自动生成，供大模型阅读）
