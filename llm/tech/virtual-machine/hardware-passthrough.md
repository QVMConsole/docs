...
> **权限与开关** 硬件直通功能仅对管理员开放，需要管理员权限才能配置直通设备。此外，硬件直通受系统设置中的 `hardware_passthrough_enabled` 开关控制（默认关闭），未开启时相关功能入口不可用。
...
| **vendor\_id** | 设置为非 KVM 标识       |
| **kvm hidden** | 启用 KVM 隐藏模式       |

> **提示** QVMConsole 会自动为 NVIDIA GPU 配置必要的隐藏参数，大多数情况下无需手动处理。
...
> **注意** 硬件直通将物理设备的完全控制权交给虚拟机，如果虚拟机被入侵，直通设备可能成为攻击向量。请确保虚拟机的安全性。

---

> 原文路径：/docs/tech/virtual-machine/hardware-passthrough（本文由 QVMConsole 文档站自动生成，供大模型阅读）
