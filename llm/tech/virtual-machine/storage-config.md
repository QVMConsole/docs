...
> **前提条件** 来宾内必须安装并运行 QEMU Guest Agent；相关操作以异步任务执行（`vm_disk_guest_mount` / `vm_disk_provision`），进度可在任务中心查看。
...
> 原文路径：/docs/tech/virtual-machine/storage-config（本文由 QVMConsole 文档站自动生成，供大模型阅读）
