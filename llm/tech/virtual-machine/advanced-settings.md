...
> **谨慎操作** 高级设置仅建议在调试或排查启动问题时使用，普通情况下请保持默认配置。错误的配置可能导致虚拟机无法启动或运行异常。
...
> **建议** 常规虚拟机建议保持启用 APIC。仅在排查非常早期的启动兼容性问题时再尝试关闭。
...
> **说明** 主要用于 x86 老系统或 32 位来宾的大内存兼容场景。非 x86 架构会自动忽略该设置。
...
> **注意** Guest Agent 需要在虚拟机内部安装并运行相应的服务才能正常工作。未安装 Agent 的虚拟机将回退到传统方式执行操作。
...
| 后端                    | 原理        | 粒度        | 兼容性  | 性能   |
| --------------------- | --------- | --------- | ---- | ---- |
| **气球调度（balloon）**     | 页面级别的内存回收 | 4KB 页     | 广泛   | 略有开销 |
| **弹性内存（virtio\_mem）** | 块级别的内存管理  | 128-512MB | 较新系统 | 更高效  |

选择弹性内存后端时会自动关闭自动气球（二者互斥）；启用气球调度要求虚拟机 XML 中已存在 virtio-balloon 设备（否则需关机后重新配置）。动态内存配置存在兼容状态机：`legacy_static`（静态内存，未启用动态）、`dynamic`（动态已生效）、`pending_apply`（配置已保存待应用，通常因虚拟机处于运行态或后端不兼容，需关机应用）。
...
> **建议** 除非有特殊需求，建议保持 SMBIOS 默认配置。修改 SMBIOS 信息可能影响某些软件的正常运行。
...
> **LTSC 模板** Windows LTSC 等模板在 Sysprep/OOBE 自动重启后可能出现黑屏，此时可选择"宿主冷启动"模式解决此问题。

---

> 原文路径：/docs/tech/virtual-machine/advanced-settings（本文由 QVMConsole 文档站自动生成，供大模型阅读）
