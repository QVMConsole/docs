...
> **配额显示** 配额值为 0 表示不限制该资源，进度条会显示为"无限制"状态。
...
> **配额设置** 配额设置为 0 表示不限制该资源，但请谨慎使用，避免资源滥用。建议根据用户实际需求设置合理的配额上限。
...
> **删除警告** 删除用户操作不可逆，所有关联资源将被永久删除。建议在删除前备份重要数据，并与用户确认。
...
> **恢复码** 启用 2FA 成功后会一次性返回 10 个恢复码（16 位，哈希存储），用于验证器丢失时登录。恢复码可通过安全中心重新生成（旧码立即失效）。
...
> **安全策略** 首次登录的安全初始化是强制性的，未完成初始化的用户无法访问系统功能。这是保障平台安全的重要措施。
...
> **设计原则** 封禁操作采用"尽力而为"策略，即使部分级联操作失败，也会完成用户封禁，确保安全策略的执行。

---

> 原文路径：/docs/tech/user-manage/user-management（本文由 QVMConsole 文档站自动生成，供大模型阅读）
