...
> **恢复码重要提示** 恢复码用于在 2FA 验证器不可用时登录账户。启用 2FA 后请立即保存恢复码，重新生成会将旧恢复码全部作废。
...
> **明文密钥仅显示一次** API Key 的明文仅在**生成 / 重新生成**的响应后保留在当前页面内存中。刷新页面或离开安全中心后无法再次查看明文，请生成后立即复制并妥善保存。
...
> **密码泄露检测** 所有输入密码的环节（含改密）都会接入 HIBP（Have I Been Pwned）密码泄露检测。若判断为高风险措施启用，泄露的密码将被拒绝，并提示更换。
...
> 原文路径：/docs/tech/user-manage/security-center（本文由 QVMConsole 文档站自动生成，供大模型阅读）
